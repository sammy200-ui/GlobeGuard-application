  ...
  const handleSubmit = async () => {
    ...
    try {
      const newScam = {
        ...
        reportedBy: "sameer_2024",
        date: new Date().toISOString().split('T')[0],
        upvotes: 1,
      };

      const existingReports = await AsyncStorage.getItem('userReports');
      let reportsArray = existingReports ? JSON.parse(existingReports) : [];
      
      reportsArray.unshift(newScam);
      await AsyncStorage.setItem('userReports', JSON.stringify(reportsArray));
